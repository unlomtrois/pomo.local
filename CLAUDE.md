...
- `$XDG_STATE_HOME/pomo/daemon.json` — `{addr, pid}` of the running daemon, written on start and removed on shutdown; `daemon stop` reads it to find the daemon whatever address it's on (falling back to `client.DefaultAddr()`).
...
- `web` — open the dashboard in the browser (`xdg-open`). Finds a running daemon via the `daemon.json` state file and opens its recorded browser URL (`pomo.local` when mDNS is on, else loopback, `:80` omitted); auto-spawns one on the default address if none is running.
