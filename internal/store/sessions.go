...
// pomo.Session, adding identity, lifecycle status, and provenance.
type Session struct {
	ID        int64         `json:"id"`
...
func (s *Store) StartSession(ctx context.Context, topic string, duration time.Duration, source string) (*Session, error) {
	start := time.Now().UTC()
	stop := start.Add(duration)

	res, err := s.db.ExecContext(ctx,
		`INSERT INTO sessions (topic, start_time, stop_time, duration, status, source)
		 VALUES (?, ?, ?, ?, ?, ?)`,
		topic, start.Format(time.RFC3339Nano), stop.Format(time.RFC3339Nano),
		int64(duration), StatusActive, source,
	)
	if err != nil {
	...
	return &Session{
		ID: id, Topic: topic, StartTime: start, StopTime: stop,
		Duration: duration, Status: StatusActive, Source: source,
	}, nil
}
...
func (s *Store) ActiveSession(ctx context.Context) (*Session, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, topic, project_id, start_time, stop_time, duration, status, source
...
func scanSession(sc rowScanner) (*Session, error) {
	...
	if err := sc.Scan(&sess.ID, &sess.Topic, &projectID, &start, &stop, &durNanos, &sess.Status, &sess.Source); err != nil {
		return nil, err
	}
	if projectID.Valid {
